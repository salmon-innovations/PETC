...
export default function SettingsPage() {
    ...
    return (_jsxs("div", { className: "max-w-xl mx-auto p-6 space-y-6", children: [_jsx("h1", { className: "text-xl font-bold text-gray-800", children: "Settings" }), _jsxs("section", { className: "bg-white rounded-xl shadow divide-y", children: [_jsx(Row, { label: "Agent Version", value: status?.agentVersion ?? "—" }), _jsx(Row, { label: "Analyzer", value: status?.analyzerConnected ? "Connected" : "Offline", ok: status?.analyzerConnected }), _jsx(Row, { label: "Printer", value: status?.printerStatus.online ? "Online" : "Offline", ok: status?.printerStatus.online }), _jsx(Row, { label: "Paper", value: status?.printerStatus.paper_ok ? "OK" : "Low / empty", ok: status?.printerStatus.paper_ok }), _jsx(Row, { label: "Cloud sync queue", value: `${status?.cloudOutboxPending ?? "—"} pending` }), _jsx(Row, { label: "Testing readiness", value: status?.readinessReady ? "Ready" : status?.readinessReason ?? "Checking", ok: status?.readinessReady })] }), _jsxs("section", { className: "bg-white rounded-xl shadow p-5 space-y-2", children: [_jsx("h2", { className: "font-semibold text-sm text-gray-700", children: "Cloud commissioning" }), _jsx("p", { className: "text-xs text-gray-500", children: status?.configured ? `${status.config.expectedCenter ?? "Configured center"}, lane ${status.config.expectedLane ?? "—"}; key ${status.config.keyMasked ?? "masked"}` : "Not configured — testing is blocked." }), _jsx(Link, { to: "/commissioning", className: "inline-block rounded border border-blue-600 px-3 py-1.5 text-xs text-blue-700 hover:bg-blue-50", children: "Reconfigure cloud lane" })] }), _jsx(AnalyzerHardwareSection, {}), _jsx(CameraHardwareSection, {}), _jsxs("section", { className: "bg-white rounded-xl shadow p-5 space-y-2", children: [_jsx("h2", { className: "font-semibold text-sm text-gray-700", children: "Data Location" }), _jsx(DataPath, {})] })] }));
}
