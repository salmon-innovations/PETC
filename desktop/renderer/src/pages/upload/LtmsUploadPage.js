...
function PhotosStep({ testId, photos, onBack, onNext }) {
    ...
    const captureMutation = useMutation({
        ...
        onError: (err) => setStatus(`Failed: ${err?.message ?? "upload error"}`),
    });
    const hasFront = photos.some((photo) => photo.photoType === "FRONT");
    return (_jsxs("section", { className: "bg-white rounded-lg shadow p-5 space-y-4", children: [_jsx(StepHeading, { title: "Step 5 - Photos" }), _jsxs("div", { className: "grid grid-cols-2 gap-4", children: [_jsxs("div", { className: "space-y-3", children: [_jsx(CameraStream, { ref: cameraRef }), _jsxs("div", { className: "flex items-center gap-3", children: [_jsx("button", { onClick: () => captureMutation.mutate("FRONT"), disabled: captureMutation.isPending, className: "rounded-md bg-blue-600 text-white px-4 py-1.5 text-sm font-medium hover:bg-blue-700 disabled:opacity-50", children: captureMutation.isPending ? "Saving…" : hasFront ? "Retake photo" : "Take photo" }), status && (_jsx("span", { className: clsx("text-xs", status.startsWith("Failed") ? "text-red-600" : "text-green-700"), children: status }))] })] }), _jsxs("div", { className: "space-y-2", children: [_jsx("p", { className: "text-sm font-semibold text-gray-700", children: "Vehicle photo" }), photos.filter((p) => p.photoType === "FRONT").length === 0 ? (_jsx("p", { className: "text-xs text-gray-500", children: "No photo yet \u2014 use the camera on the left." })) : (_jsx("div", { className: "grid grid-cols-2 gap-2", children: photos.filter((p) => p.photoType === "FRONT").map((photo) => (_jsx(PhotoThumb, { photoId: photo.id }, photo.id))) }))] })] }), _jsx(FooterNav, { nextDisabled: !hasFront, onBack: onBack, onNext: onNext })] }));
}
function PhotoThumb({ photoId }) {
    const [src, setSrc] = useState(null);
    useEffect(() => {
        let cancelled = false;
        sidecarClient.photoUrl(photoId).then((u) => { if (!cancelled)
            setSrc(u); });
...
export function CecPreviewAndPrint({ submissionId, certificateNo, onDone }) {
    ...
    return (_jsxs("section", { className: "bg-white rounded-lg shadow overflow-hidden", children: [_jsxs("div", { className: "flex items-center justify-between gap-4 border-b border-gray-200 bg-green-50 px-5 py-3", children: [_jsxs("div", { children: [_jsx("p", { className: "text-sm font-bold text-green-800", children: "CEC issued \u2014 review before printing" }), certificateNo && (_jsxs("p", { className: "text-xs text-green-700", children: ["Certificate No: ", _jsx("strong", { children: certificateNo })] }))] }), _jsxs("div", { className: "flex items-center gap-2", children: [printStatus === "printed" && _jsx("span", { className: "text-xs text-green-700", children: "Sent to printer \u2713" }), printStatus === "error" && _jsx("span", { className: "text-xs text-red-600", children: "Print failed" }), _jsx("button", { onClick: () => printMutation.mutate(), disabled: printMutation.isPending, className: "rounded-md bg-blue-600 px-4 py-1.5 text-xs font-medium text-white hover:bg-blue-700 disabled:opacity-50", children: printMutation.isPending ? "Printing…" : "Print CEC ×2" }), _jsx("button", { onClick: onDone, className: "rounded-md border border-gray-300 px-4 py-1.5 text-xs font-medium hover:bg-gray-50", children: "Done" })] })] }), _jsx("div", { className: "h-[70vh] bg-gray-100", children: pdfUrl ? (_jsx("iframe", { src: pdfUrl, title: "CEC preview", className: "w-full h-full border-0" })) : (_jsx("div", { className: "flex h-full items-center justify-center text-sm text-gray-500", children: "Loading CEC PDF\u2026" })) })] }));
}
