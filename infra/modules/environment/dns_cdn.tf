...
resource "aws_cloudfront_origin_access_control" "s3" {
  ...
  description                       = "Private S3 origins for ${local.name}"
...
resource "aws_cloudfront_distribution" "frontend" {
  ...
  comment             = "${local.name} frontend, API proxy, and desktop releases"
  ...
  depends_on = [aws_route53_record.api, aws_lb_listener.https]
  tags       = local.common_tags
}
